...
import CampaignAIReport from './CampaignAIReport';
...
export default function ObjectiveTabs({ reportData, campaigns, campaignNameMap, campaignObjectiveMap, currentRange, accounts, selectedAccounts, loading }) {
  ...
  function downloadHTML() {
    const el = document.getElementById('perf-per-campaign-content');
    if (!el) return;
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Performance per Campaign - ${accountName}</title>
<style>body{font-family:Helvetica,Arial,sans-serif;background:#F4F3F0;color:#272828;padding:32px;max-width:1400px;margin:0 auto}
table{width:100%;border-collapse:collapse;margin-bottom:24px}th,td{text-align:left;padding:8px 12px;border-bottom:1px solid #e8e6df;font-size:12px}
th{background:#f2f1ec;font-family:monospace;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#8a8880}
@media print{body{background:white}}</style>
</head><body>
<h1 style="font-size:24px;margin-bottom:4px">${accountName}</h1>
<p style="color:#888;margin-bottom:32px">Performance per Campaign &nbsp;·&nbsp; ${currentRange?.start} to ${currentRange?.end}</p>
${el.innerHTML}
</body></html>`;
    ...
    return (
      <div className="bg-slate-800 rounded-xl border border-slate-700 p-16 text-center">
        ...
        <p className="text-slate-400 text-sm max-w-sm mx-auto">
          Select an account and date range in the Report Generator tab first, then return here to see performance per campaign.
        </p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {/* Campaign cards */}
      <div id="perf-per-campaign-content">
